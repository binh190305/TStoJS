...
// Removed interface TableManagementProps
export function TableManagement({ onPageChange }) { // Removed type from props
  ...
  const [editingTable, setEditingTable] = useState(null); // Removed <any>
  ...
  const handleEdit = (table) => { // Removed type annotation
  ...
  const handleSave = () => {
    // Note: Parsing the rate ensures it's treated as a number
    const rateAsNumber = parseInt(formData.hourlyRate, 10);

    if (editingTable) {
      setTables(tables.map(table => table.id === editingTable.id ? { ...formData, id: editingTable.id, hourlyRate: rateAsNumber } : table));
    } else {
      setTables([...tables, { ...formData, id: Date.now().toString(), hourlyRate: rateAsNumber }]);
    }
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => {
                    // Always try to parse input as number, default to 0 if invalid
                    const rate = parseInt(e.target.value, 10);
                    setFormData({ ...formData, hourlyRate: isNaN(rate) ? 0 : rate });
                }}
