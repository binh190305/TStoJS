...
export function BillManagement({ onPageChange }: BillManagementProps) {
  ...
  const handleTableOpen = (table: any) => {
    setSelectedTable(table);
    setActiveTab('order');
  };

  const addToOrder = (product: any) => {
    const existingItem = currentOrder.find((item: any) => item.id === product.id);
    if (existingItem) {
  ...
  const removeFromOrder = (productId: string) => {
    setCurrentOrder(currentOrder.filter((item: any) => item.id !== productId));
  };

  const calculateTotal = () => {
    return currentOrder.reduce((total: number, item: any) => total + (item.price * item.quantity), 0);
  };
