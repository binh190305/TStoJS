...
// Import các biểu tượng cần thiết từ lucide-react
import { Eye, EyeOff, Loader2 } from 'lucide-react';

// --- UI Component Stubs (Tailwind CSS based for single-file execution) ---
...
// Input Component
const Input = ({ id, name, type = 'text', value, onChange, placeholder, className = '', required = false }) => (
  <input
    id={id}
    name={name}
    type={type}
...
// Mock Auth Context/Hook (since the real provider is not available)
const useAuth = () => ({
    // Mock function - in a real app, this would handle Firebase/OAuth login
    loginWithGoogle: () => {
...
export default function SignUp({ onNavigate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Attempting mock sign up:', formData);
      await new Promise(resolve => setTimeout(resolve, 1000)); // Simulate API call
      onNavigate('signin');
    } catch (err) {
      // Dùng console.error thay vì log đơn giản
